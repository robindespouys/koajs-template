...
describe('Create Things(s) on /things route', () => {
  ...
  describe('Create a valid Thing', () => {
    it('Should return status 201 and a representation of the Thing', async () => {
      const thingCreation: any = await request.agent(server.app).post('/things').set({ 'authorization': `Bearer ${createdUserToken}` }).send({
        "name": "basic thing",
...
describe('Update Things(s) on /things route', () => {
  ...
  describe('Update Thing with good parameters', () => {
    it('Should return status 200 and the updated Thing', async () => {
      const thingUpdate: any = await request.agent(server.app).put(`/things/${createdThing.id}`).set({ 'authorization': `Bearer ${createdUserToken}` }).send({
        "name": "new thing",
      ...
      expect(thingUpdate.body.description).to.be.equal('same thing but different');
    });
  });
...
describe('Retrieve Things(s) on /things route', () => {
  ...
  describe('Retrieve an existing Thing', () => {
    it('Should return status 200 and the updated Thing', async () => {
      const thingUpdate: any = await request.agent(server.app).get(`/things/${createdThing.id}`).set({ 'authorization': `Bearer ${createdUserToken}` });
  ...
  describe('Update Thing with good parameters', () => {
    it('Should return status 200 and an Array containing representations of the Things', async () => {
      const thingUpdate: any = await request.agent(server.app).get('/things').set({ 'authorization': `Bearer ${createdUserToken}` });
      expect(thingUpdate.status).to.equal(200);
...
describe('Delete Things(s) on /things route', () => {
  ...
  describe('Delete the previously created Thing', () => {
    it('Should return status 204 and Thing id deleted message', async () => {
      const thingDeletion: request.Response = await request.agent(server.app).delete(`/things/${createdThing.id}`).set({ 'authorization': `Bearer ${createdUserToken}` });
      expect(thingDeletion.status).to.equal(200);
      expect(thingDeletion.text).to.be.equal(`Thing ${createdThing.id} deleted`);
    });
  });
