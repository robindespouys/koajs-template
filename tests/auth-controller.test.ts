...
describe('Sign-up on /auth/signup route', () => {
  ...
  describe('Sign-in with missing fields', () => {
    it('Should return status 400 and a list of missing fields', async () => {
      const userRegistration: any = await request.agent(server.app).post('/auth/signup').send({ 'email': 'super@domain.com' });
      expect(userRegistration.status).to.be.equal(400);
      expect(userRegistration.text).to.be.equal('Missing fields : password,name');
    });
  });
  ...
  describe('Sign-up with bad email inputs', () => {
    it('Should return status 400 and email format verification error in the body', async () => {
      const userRegistration: any = await request.agent(server.app).post('/auth/signup').send({ 'email': 'super___domain.com', 'password': '12345', 'name': 'coolname' });
      expect(userRegistration.status).to.be.equal(400);
  ...
  describe('Sign-up with already existing email', () => {
    it('Should return status 400 and an already used email adress message', async () => {
      const userRegistration: any = await request.agent(server.app).post('/auth/signup').send({ 'email': 'super@domain.com', 'password': '12345', 'name': 'coolname' });
      expect(userRegistration.status).to.be.equal(400);
      expect(userRegistration.text).to.be.equal('The specified e-mail address already exists');
    });
  });
...
describe('Sign-in on /auth/signin route', () => {
  ...
  describe('Sign-in with missing fields', () => {
    it('Should return status 400 and a list of missing fields', async () => {
      const userAuthentication: any = await request.agent(server.app).post('/auth/signin').send({ 'email': 'super_not_existing@domain.com' });
  ...
  describe('Sign-in with incorect email', () => {
    it('Should return status 404 and a user does not exist error', async () => {
      const userAuthentication: any = await request.agent(server.app).post('/auth/signin').send({ 'email': 'super_not_existing@domain.com', 'password': '12345' });
  ...
  describe('Sign-in with incorect password', () => {
    it('Should return status 401 and an incorrect password error', async () => {
      const userAuthentication: any = await request.agent(server.app).post('/auth/signin').send({ 'email': 'super@domain.com', 'password': '00000' });
  ...
  describe('Sign-in with good credentials email', () => {
    it('Should return status 200 and user/token in the body', async () => {
      const userAuthentication: any = await request.agent(server.app).post('/auth/signin').send({ 'email': 'super@domain.com', 'password': '12345' });
...
describe('Sign-out on /auth/signout route', () => {
  ...
  describe('Sign-out without Json Web Token', () => {
    it('Should return status 200 and not yet implemented message', async () => {
      const userSignout: any = await request.agent(server.app).post('/auth/signout');
  ...
  describe('Sign-out with a valid Json Web Token', () => {
    it('Should return status 200 and not yet implemented message', async () => {
      const userSignout: any = await request.agent(server.app).post('/auth/signout').set({ 'authorization': `Bearer ${createdUserToken}` });
